...
public class NativeEnvironmentStepCatalogueTests
{
    ...
    public async Task KubernetesEnvironment_RegistersExpectedSteps()
    {
        ...
        using var app = builder.Build();
        var model = app.Services.GetRequiredService<DistributedApplicationModel>();
        var environment = model.Resources.Single(r => r.Name == "k8s-env");

        var steps = await PipelineSpikeHarness.ResolveStepsAsync(app, environment);

        // Modeling steps (must keep running under a Pulumi backend).
        Assert.Contains(steps, s => s.Name == "prepare-deployment-targets-k8s-env" && s.RequiredBySteps.Contains(WellKnownPipelineSteps.BeforeStart));
        Assert.Contains(steps, s => s.Name == "publish-k8s-env" && s.RequiredBySteps.Contains(WellKnownPipelineSteps.Publish));

        // Execution steps (suppression targets: everything Helm).
        Assert.Contains(steps, s => s.Name == "check-helm-prereqs-k8s-env");
        Assert.Contains(steps, s => s.Name == "prepare-k8s-env" && s.DependsOnSteps.Contains(WellKnownPipelineSteps.Publish));
        Assert.Contains(steps, s => s.Name == "helm-deploy-k8s-env" && s.Tags.Contains("helm-deploy") && s.RequiredBySteps.Contains(WellKnownPipelineSteps.Deploy));
        Assert.Contains(steps, s => s.Name == "destroy-helm-k8s-env" && s.RequiredBySteps.Contains(WellKnownPipelineSteps.Destroy));
        Assert.Contains(steps, s => s.Name == "helm-uninstall-k8s-env" && s.Tags.Contains("helm-uninstall"));
    }

    [Fact]
    public async Task DockerComposeEnvironment_RegistersExpectedSteps()
    {
        var builder = PipelineSpikeHarness.CreatePublishBuilder(out _);
        ...
        using var app = builder.Build();
        var model = app.Services.GetRequiredService<DistributedApplicationModel>();
        var environment = model.Resources.Single(r => r.Name == "compose-env");

        var steps = await PipelineSpikeHarness.ResolveStepsAsync(app, environment);

        // Modeling steps.
        Assert.Contains(steps, s => s.Name == "prepare-deployment-targets-compose-env" && s.RequiredBySteps.Contains(WellKnownPipelineSteps.BeforeStart));
        Assert.Contains(steps, s => s.Name == "publish-compose-env" && s.RequiredBySteps.Contains(WellKnownPipelineSteps.Publish));

        // Execution steps (suppression targets: docker compose up/down).
        Assert.Contains(steps, s => s.Name == "prepare-compose-env" && s.DependsOnSteps.Contains(WellKnownPipelineSteps.Publish));
        Assert.Contains(steps, s => s.Name == "docker-compose-up-compose-env" && s.Tags.Contains("docker-compose-up") && s.RequiredBySteps.Contains(WellKnownPipelineSteps.Deploy));
        Assert.Contains(steps, s => s.Name == "destroy-compose-compose-env" && s.RequiredBySteps.Contains(WellKnownPipelineSteps.Destroy));
        Assert.Contains(steps, s => s.Name == "docker-compose-down-compose-env" && s.Tags.Contains("docker-compose-down"));
    }
}
